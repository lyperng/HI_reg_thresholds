...
write.csv(FOSS1, 'data/HCC cleaned/FOSS with Millions.csv', row.names = F)

FOSS_dryad<-join(dryad2, FOSS1, by = 'Year', type = 'inner')
#only 18 of data years left-- see if it's enough for GAM analyses
#if not, may have to creatively pad the dryad data

write.csv(FOSS_dryad, 'data/HCC cleaned/FOSS dryad.csv', row.names = F)
...
rec_mod<-lm(RCatch~Angler.Trips ,data = recr)
summary(rec_mod)
plot(RCatch~Angler.Trips ,data = recr)
plot(RCatch ~ Year, data = recr)
plot(Angler.Trips ~ Year, data = recr)
#############################################
...
FOSS<-read.csv('data/HCC cleaned/FOSS with Millions.csv') 
...
commcatch<-gam(Catch.Millions ~ s(Year, k = 9), sp = 0.001,
             data = FOSS, method = 'REML', correlation=corAR1(form=~Year))
commcatch1<-gam(Catch.Millions ~ s(Year, k = 9), sp = 0.001,
                data = FOSS, method = 'REML')
AIC(commcatch,commcatch1)

commcatch$sp
gam.check(commcatch)

png('figures/HCC/Commercial Catch Assumptions.png', width =  1500, height = 1000, #w & h in pixels
    res = 200) #sets high resolution--default is 150
appraise(commcatch) #much better!
dev.off()
...
png('figures/HCC/Commercial Catch GAM.png', width =  2400, height = 1700, #w & h in pixels
...
ggsave('figures/HCC/Commercial Catch FD.png', 
...
ggsave('figures/HCC/Commercial Catch SD.png', 
       width =  8.47, height = 5, units = 'in', #w & h in inches
       dpi = 300, bg = 'transparent')
...
datasets<-c('data/HCC cleaned/FOSS dryad.csv', 'data/HCC cleaned/Recreation dryad.csv', 
...
trips<-gam(log(Trips.Millions) ~ s(Algae, k =7) + s(Predators, k =7), 
              data = rec0, method = 'REML', correlation=corAR1(form=~Year))
#no collinearity is an assumption--maybe can't use related predictors? e.g. predators & herbivores

trips1<-gam(log(Trips.Millions) ~ s(Algae, k =7) + s(Predators, k =7), 
           data = rec0, method = 'REML')
...
rcatch<-gam(RCatch.Millions ~ s(Year, k =7), sp = 0.0001,
             data = rec.Y, method = 'REML', correlation=corAR1(form=~Year))

appraise(rcatch) #does not meet assumptions-->looks like an outlier (probably Year 2003)
